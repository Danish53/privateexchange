...
/**
 * Assigns automatic membership from portfolio USD: **sticky** — never downgrade below a tier the user
 * already holds on an automatic assignment; upgrades when balance qualifies for a higher tier.
 * Skips if the user has a **manual** assignment. Sets `isVip` when tier reaches the first step above entry (min 0).
 * @param {string|import('mongoose').Types.ObjectId} userId
...
export async function syncAutomaticMembershipForUser(userId, opts = {}) {
  ...
  if (existing && String(existing.assignmentType || '').toLowerCase() === 'manual') {
  ...
  if (existing && String(existing.assignmentType || '').toLowerCase() === 'automatic' && existing.membershipTier) {
    currentTier = await MembershipTier.findById(existing.membershipTier).lean();
  }

  let best = qualifiedBest;
  if (currentTier) {
    const curMin = Number(currentTier.minValueUsd) || 0;
    const qualMin = qualifiedBest ? Number(qualifiedBest.minValueUsd) || 0 : -Infinity;
    if (curMin > qualMin) {
      best = currentTier;
    } else if (!qualifiedBest) {
      best = currentTier;
    } else {
      best = qualifiedBest;
    }
  ...
  const sameTier =
    existing &&
    String(existing.assignmentType || '').toLowerCase() === 'automatic' &&
    String(existing.membershipTier) === String(best._id);
