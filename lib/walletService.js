...
function formatAmount(n) {
  return formatNumberSmart(n, { locale: 'en-US', minFractionDigits: 0, maxFractionDigits: 2 });
}
...
/**
 * Convert USD amount to token amount with precise calculation.
 * Uses fixed-point arithmetic to avoid floating point errors.
 * @param {number} usdAmount - USD amount
 * @param {number} usdPerUnit - Conversion rate (USD per token)
 * @returns {number} Token amount
 */
function usdToTokens(usdAmount, usdPerUnit) {
  if (usdPerUnit === 1) {
...
export async function ensureWalletForMemberUser(userId) {
  ...
  for (const t of tokens) {
    await WalletTokenBalance.updateOne(
      ...
      { $setOnInsert: { wallet: wallet._id, token: t._id, balance: 0, purchasedBalance: 0 } },
      { upsert: true }
    );
  }
...
export async function getWalletSummaryForUserId(userId) {
  ...
  const totalUsd = Number(usdRow?.balance) || 0;
  const usdTokenBalance = totalUsd;
  ...
  let portfolioUsd = 0;
  const tokens = sortedRows.map((r) => {
    const bal = Number(r.balance) || 0;
    const purchasedBal = Number(r.purchasedBalance) || 0;
    const unit = Number(r.token.usdPerUnit) || 1;
    const usdValue = bal * unit;
    ...
    return {
      ...
      purchasedBalance: formatAmount(purchasedBal),
      value: formatAmount(bal),
  ...
  return {
    ...
    usdTokenBalance,
    usdTokenBalanceFormatted: formatUsd(usdTokenBalance),
    portfolioUsd,
    portfolioUsdFormatted: formatUsd(portfolioUsd),
  };
}
...
export async function applyAdminWalletAdjustment({
  ...
  // First try to find token in database
  let tokenDoc = await Token.findOne({
    $or: [
      { symbol: { $regex: new RegExp(`^${symIn}$`, 'i') } },
      { slug: { $regex: new RegExp(`^${symIn}$`, 'i') } }
    ],
    isActive: true
  }).lean();
  ...
  if (!tokenDoc) {
    // Fallback to PLATFORM_TOKEN_SEED for backward compatibility
    const seed = PLATFORM_TOKEN_SEED.find((t) => String(t.symbol).toUpperCase() === symIn);
    if (!seed || !LEDGER_SYMBOLS.has(symIn)) {
      return { ok: false, error: 'INVALID_TOKEN' };
    }
    // Find token by slug from seed
    const tokenBySlug = await Token.findOne({ slug: seed.slug, isActive: true }).lean();
    if (!tokenBySlug) {
      return { ok: false, error: 'TOKEN_NOT_FOUND' };
    }
    // Use tokenBySlug as tokenDoc
    tokenDoc = tokenBySlug;
    seedSlug = seed.slug;
  } else {
    seedSlug = tokenDoc.slug;
  }
  ...
  // tokenDoc is already defined from earlier logic
  if (!tokenDoc) {
    return { ok: false, error: 'TOKEN_NOT_FOUND' };
  }
  ...
  try {
    const { syncMembershipAfterWalletChange } = await import('@/lib/userMembershipAssignmentService');
    await syncMembershipAfterWalletChange(targetUserId, { portfolioUsd: summary.portfolioUsd });
  } catch (e) {
    console.error('applyAdminWalletAdjustment syncMembershipAfterWalletChange', e);
  }
