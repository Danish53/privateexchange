...
import {
  ...
  userHasTransactionsView,
  userHasDrawingsManage,
  userHasMembershipManage,
  userHasAnnouncementsManage,
  userHasSupportTicketsManage,
...
/** @param {(user: Record<string, unknown>) => boolean} checker */
async function requirePlatformModule(request, checker) {
  const act = await loadRequestActor(request);
  if ('error' in act) return act;
  if (!checker(act.user)) {
    return { error: NextResponse.json({ ok: false, error: 'Forbidden' }, { status: 403 }) };
...
export function requireTransactionsView(request) {
  return requirePlatformModule(request, userHasTransactionsView);
}

export function requireDrawingsManage(request) {
  return requirePlatformModule(request, userHasDrawingsManage);
}

export function requireMembershipManage(request) {
  return requirePlatformModule(request, userHasMembershipManage);
}

export function requireAnnouncementsManage(request) {
  return requirePlatformModule(request, userHasAnnouncementsManage);
}

export function requireSupportTicketsManage(request) {
  return requirePlatformModule(request, userHasSupportTicketsManage);
}
