...
async function backfillUserVipFlagOnce() {
  if (global.__userVipBackfillDone) return;
  global.__userVipBackfillDone = true;
  try {
    ...
    if ((res?.modifiedCount || 0) > 0) {
      console.log(`[MongoDB] Backfilled users.isVip=false for ${res.modifiedCount} existing users`);
    }
  } catch (e) {
    console.error('[MongoDB] users.isVip backfill failed:', e?.message || e);
  }
}
