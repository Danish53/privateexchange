...
export async function provisionVerifiedUser(request, input) {
  ...
  const doc = {
    ...
    name,
    role,
    isVip: role === 'user' ? isVip : false,
    emailVerified: true,
    verificationOtpHash: null,
    verificationOtpExpires: null,
    deletedAt: null,
  };
  if (role === 'admin' && input.adminPermissions && typeof input.adminPermissions === 'object') {
    doc.adminPermissions = mergeAdminPermissions(input.adminPermissions);
  }
